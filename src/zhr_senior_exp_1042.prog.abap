...
FORM f_export.
  ...
  DATA: gt_dep   TYPE STANDARD TABLE OF ty_dep,
        gt_wtext TYPE STANDARD TABLE OF ty_wtext.

  gv_filename = sy-datum && '_SENIOR_1042_' && gv_count &&'.csv'.
  ...
  LOOP AT gt_pay ASSIGNING FIELD-SYMBOL(<fs_pay>).
    ...
    LOOP AT lt_rt ASSIGNING FIELD-SYMBOL(<fs_rt>).
      ...
      " zerar o contador de tempo de execução
    ...
    if lines( gt_file ) gt 100000.
      ...
      USING gv_filename CHANGING gt_file p_locl p_serv.
...
FORM f_build_calc_map
  ...
  SORT gt_calc BY bukrs tipcal perref paydt fpbeg fpend.
  DELETE ADJACENT DUPLICATES FROM gt_calc COMPARING bukrs tipcal perref
paydt fpbeg fpend.
